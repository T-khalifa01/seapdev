...
const allowedSubscriptionsColumns = [
  "id", "email", "type", "created_at"
];

const checkColumn = (key) => {
  if (allowedSubscriptionsColumns.includes(key)) {
    return key;
  }
...
// =================================================================
const validateSubscriptionData = (data, isPatch = false) => {
  const errors = {};
  
  // Email is the only required field for a new subscription
...
export async function GET(request) {
  try {
    ...
    const searchCondition = search ? `
      WHERE
        email LIKE ? OR type LIKE ?
    ` : '';
    const searchParam = search ? [`%${search}%`, `%${search}%`] : [];
    
    const countQuery = `SELECT COUNT(*) AS total_count FROM subscriptions ${searchCondition}`;
...
export async function POST(request) {
  try {
    ...
    const keys = Object.keys(data).filter(key => allowedSubscriptionsColumns.includes(key) && key !== 'id');
    ...
    const [newRowResult] = await pool.query('SELECT * FROM subscriptions WHERE id = ?', [newId]);
    
    return NextResponse.json({ success: true, row: newRowResult[0] });

  } catch (error) {
    console.error('API POST Error:', error);
    return NextResponse.json({ error: 'Failed to create new subscription record' }, { status: 500 });
  }
}
...
export async function PATCH(request) {
  ...
  try {
    ...
    const errors = validateSubscriptionData({ [safeKey]: value }, true);
    if (Object.keys(errors).length > 0) {
      return NextResponse.json({ error: 'Validation failed', fields: errors }, { status: 400 });
    }

    //  1.fetch
    const [originalResult] = await pool.query('SELECT * FROM subscriptions WHERE id = ?', [id]);
    ...
    //   new_value: value,
    // };
    // await logDataChange('edited', 'subscriptions', id, originalRow, changedBy);
    ...
    //  3. fetch
    const [updatedRowResult] = await pool.query('SELECT * FROM subscriptions WHERE id = ?', [id]);
    const updatedRow = updatedRowResult[0];
    // 4. Log the full original and updated rows to the audit log
    await logDataChange('edited', 'subscriptions', id, originalRow, changedBy, updatedRow);
    //
...
export async function DELETE(request) {
  ...
  try {
    ...
    //  2. log
    await logDataChange('deleted', 'subscriptions', id, originalRow, changedBy);
    ...
    return NextResponse.json({ error: 'Failed to delete subscription record' }, { status: 500 });
  }
}